...
/// Screen for creating a new route with name, description, and waypoints.
...
class _RouteCreateScreenState extends ConsumerState<RouteCreateScreen> {
  ...
  final List<({double lat, double lon})> _waypoints = [];
          ...
          children: [
            FormFieldCard(
              ...
              child: TextFormField(
                ...
                decoration:
                    const InputDecoration(hintText: 'Enter route name'),
                validator: (v) =>
            ...
            FormFieldCard(
              ...
              description:
                  'Paste GeoJSON geometry or add waypoints below.',
              child: TextFormField(
                controller: _geometryController,
                decoration: const InputDecoration(
                    hintText: '{"type":"LineString",...}'),
                maxLines: 4,
                style:
            ...
            FormFieldCard(
              label: 'Waypoints',
              description:
                  'Add latitude/longitude pairs for route waypoints.',
              child: Column(
  ...
  void _addWaypoint() {
    ...
    setState(() {
      _waypoints.add((lat: lat, lon: lon));
  ...
  Future<void> _submit() async {
    ...
    try {
      final created =
          await ref.read(routeNotifierProvider.notifier).createRoute(route);
      if (mounted) {
        context.go('/geo/routes/${created.id}');
      }
    } catch (e) {
      if (mounted) {
